        ...
        case let .unauthorizedBasicAuth(realm):
            if let realm = realm {
                return Response(
                    ...
                    content: PlainText("Unauthorized")
                )
            } else {
                return Response(status: .unauthorized, content: PlainText("Unauthorized"))
            }
        case .unauthorized:
            return Response(status: .unauthorized, content: PlainText("Unauthorized"))
        case .noAuthorizationHeader:
            return Response(status: .unauthorized, content: PlainText("Authorization header missing."))
        }
    }
}

public struct Authenticator {
    public typealias AuthenticateBasicAuth = (_ request: Request, _ username: String, _ password: String) throws -> Void
    ...
    public typealias Authenticate = (_ request: Request, _ scheme: String, _ payload: String) throws -> Void
